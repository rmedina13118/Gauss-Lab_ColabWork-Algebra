...
function solveGaussJordan(aug: Matrix, nVars: number) {
  ...
  const typeEs = type === "UNIQUE" ? "Solucion unica" : type === "INFINITE" ? "Infinitas soluciones" : "Ninguna solucion"
...
const LEVELS_DATA = [
  ...
  },
  {
    ...
    story: "El reactor solo se estabiliza si encontras una solucion exacta para x e y.",
...
export function JavaPrototypeSection() {
  ...
  const solveLevel = useCallback((lvlIdx: number, method: number) => {
    ...
    const guessMap: Record<string, string> = { "1": "Solucion unica", "2": "Infinitas soluciones", "3": "Ninguna solucion" }
                      ...
                      {LEVELS_DATA.map((_, i) => (
                        <div
                          ...
                          className={`w-6 h-1.5 rounded-full transition-all duration-500 ${
                            i < currentLevel ? "bg-[hsl(150,100%,50%)]"
                      ...
                      {["a11", "a12", "b1", "a21", "a22", "b2"].map((label, i) => (
                        <div
                          ...
                          className={`px-1.5 py-0.5 rounded text-[8px] font-mono transition-all ${
                            i < customCoefs.length ? "bg-[hsl(150,100%,50%,0.15)] text-[hsl(150,100%,50%)] border border-[hsl(150,100%,50%,0.3)]"
